...
async function roundSteal_timeout(room, gs, rQs) {
  ...
  await fp(`rooms/${CODE}`, { "gameState/phase":"questionResult", "gameState/revealed":true, "gameState/result":{ msg:`⏱️ Temps écoulé ! Bonne réponse : ${q.a[q.c]}`, pts:0, scorer:null } });
...
async function roundSteal_end(room, gs, rQs) {
  ...
  if (correct.length === 0) {
    if (HTIMER) { clearTimeout(HTIMER); HTIMER = null; }
    await fp(`rooms/${CODE}`, { "gameState/phase":"questionResult", "gameState/revealed":true, "gameState/result":{ msg:`❌ Personne ! Bonne réponse : ${q.a[q.c]}`, pts:0, scorer:null } });
    setTimeout(() => hostNextQ(room, gs, rQs), 4500);
  }
}
...
async function roundSteal_pick(room, gs, rQs, targetName) {
  ...
  await fp(`rooms/${CODE}`, { "gameState/phase":"questionResult", "gameState/revealed":true, "gameState/pickTarget":false, "gameState/scores":sc, "gameState/result":{ msg:`😈 ${gs.buzzed} vole ${actualStolen} pts à ${targetName} !`, pts:actualStolen, scorer:gs.buzzed } });
  setTimeout(() => hostNextQ(room, gs, rQs), 4500);
}
